...
def split_and_scale_features(X_all, scaler_color):
    n_color, n_asym, n_border = 12, 2, 4
    X_color = X_all[:, :n_color]
    X_asym = X_all[:, n_color:n_color + n_asym]
    X_haralick = X_all[:, n_color + n_asym:n_color + n_asym]
    X_border = X_all[:, n_color + n_asym:]
...
def prepare_test_data(feature_columns, scaler_color):
    ...
    for df in [df_test_asym, df_test_border]:
        df["img_id"] = df["img_id"].str.replace(".png", "")
    ...
    X_test_haralick = X_test[:, n_color + n_asym:n_color + n_asym]
    X_test_border = X_test[:, n_color + n_asym:]

    X_test_color_scaled = scaler_color.transform(X_test_color)
    X_test_final = np.hstack([X_test_color_scaled, X_test_asym, X_test_border])
